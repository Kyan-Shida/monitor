...
import type { Result, Robot, Task } from "../data/types";
...
/**
 * 待办条目：左侧承载业务信息，右侧承载处理动作
 * @param children 业务信息区内容
 * @param actions 处理动作区内容
 */
function Todo({
  children,
  actions,
...
export function Workbench() {
  ...
  // 派单选择：任务 ID → 机器人 ID，未选择时回退到首个满足条件的机器人
  const [picked, PICKED] = useState<Record<string, string>>({});
  ...
  /**
   * 机器人下拉选项文案：下拉宽度有限，仅保留可用时刻，省去括号内的相对时长
   * @param r 机器人
   * @returns 形如 12:35 或 现在 的可用时刻
   */
  const availableAt = (r: Robot) => robotAvailability(s, r).split("（")[0];

  /**
   * 在当前页对任务派单：优先使用人工选择的机器人，否则取首个满足约束的机器人
   * @param t 待调度任务
   * @returns 无可用机器人时不执行任何动作
   */
  function assign(t: Task) {
    const robotId = picked[t.id] || availableFor(t)[0]?.id;
    if (robotId) act({ type: "ASSIGN", id: t.id, robotId });
  }

  /**
   * 计算当前满足调度约束的机器人；地图/点位版本差异由平台自动安排同步窗口，不阻塞派单
   * @param t 待调度任务
   * @returns 可承接该任务的机器人列表
   */
  function availableFor(t: Task): Robot[] {
    return s.robots.filter(
      (r) =>
  ...
  /**
   * 打开复核弹窗，并以当前结果的识别值作为默认表单内容
   * @param r 待复核结果
   */
  function openReview(r: Result) {
    REVIEWING(r);
    CONC("确认");
    VAL(r.final);
    FS(r.abnormal ? "异常" : "正常");
              ...
              {pendingResults.map((r) => {
                ...
                return (
                  <Todo
                    ...
                    actions={
                      <>
                        <Btn primary onClick={() => openReview(r)}>
                          复核
                        </Btn>
                        <Btn onClick={() => go("review", r.id)}>查看证据</Btn>
                      </>
                    }
                  >
                    <b>
        ...
        <div id="wb-tasks">
          ...
          >
            <ul className="todo-list">
              ...
              {pendingTasks.map((t) => {
                const robots = availableFor(t);
                const selected = picked[t.id] || robots[0]?.id || "";
                return (
                  <Todo
                    key={t.id}
                    actions={
                      <>
                        <select
                          aria-label={`为 ${t.name} 选择机器人`}
                          value={selected}
                          disabled={!robots.length}
                          onChange={(e) =>
                            PICKED((old) => ({ ...old, [t.id]: e.target.value }))
                          }
                        >
                          {!robots.length && <option>暂无可用机器人</option>}
                          {robots.map((r) => (
                        ...
                        <Btn onClick={() => go("dispatch", t.id)}>调度详情</Btn>
                      </>
                    }
                  >
                    <b>
            ...
            <ul className="todo-list">
              ...
              {robotAttention.map(({ robot, reasons }) => (
                <Todo
                  ...
                  actions={
                    <>
                      <Btn onClick={() => go("robot", robot.id)}>机器人详情</Btn>
                      {reasons.some((x) => x.startsWith("地图版本落后")) && (
                        <Btn
                          onClick={() => go("maps", robot.mapId, "sync")}
                        >
                          安排同步
                        </Btn>
                      )}
